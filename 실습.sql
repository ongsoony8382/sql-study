...
FROM employees;
-- 이름보다 카디널리티가 높으므로 앞에 오도록 인덱스 생성
...
CREATE INDEX I_hiredate_firstname
    ON employees (hire_date, first_name);

DROP INDEX I_hiredate_firstname ON employees;
...
/*I_출입문 존재: 5.453초*/ -- 불필요한 인덱스는 인서트 업데이트 딜리트의 속도를 감소시킴
/*I_출입문 삭제: 0.434초*/
...
-- gender, last_name
EXPLAIN ANALYZE
SELECT emp_no, first_name, employees.last_name
FROM employees
WHERE gender = 'M'
  AND last_name = 'Baba';

-- 기존 인덱스 삭제, 새로운 인덱스 추가
...
-- hire_date에 인덱스가 걸려있음에도 옵티마이저가 선택한 방식 -> ALL ( 풀 스캔 )
EXPLAIN
...
-- 2. first_name 값을 모두 소문자로 변경하여 lower_first_name으로 값을 넣고 싶다.
UPDATE employees
